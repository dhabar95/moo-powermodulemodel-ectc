...
class initial_param_MOO_3_obj:
    
    def __init__(self, ax_client, MOO_funtion, param_dict, first_objective_name, second_objective_name, third_objective_name):
    ...
    def evaluation_function(self, parameterization):
        x = np.array([parameterization.get(f"x{i+1}") for i in range(14)])
        multi_obj = self.MOO_funtion(*x)
        # In our case, standard error is 0, since we are computing a synthetic function.
        return {self.first_objective_name: (multi_obj[0], 0.0), self.second_objective_name: (multi_obj[1], 0.0), self.third_objective_name: (multi_obj[2], 0.0)}
